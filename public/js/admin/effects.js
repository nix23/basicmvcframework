effects = {
	...
	spinner: {
		...
		callback: function()
		{
			...
			{
				case "hide_settings_wrapper":
					form_tools.settings.hide_wrapper();
				break;
			}
		...
		toggle: function( spinner_id, 
								...
								spinner_wrapper_div,
								is_update_spinner_top_required)
		{
			if(is_update_spinner_top_required == "no")
				is_update_spinner_top_required = false;
			else
				is_update_spinner_top_required = true;

			this.spinner = $("#" + spinner_id);
			this.wrapper = $(this.spinner).children(".wrapper")
			
			// Initialize grow
			if($(this.spinner).css("visibility") == "hidden")
			{
				if(is_update_spinner_top_required)
				{
					// Updating spinner top
