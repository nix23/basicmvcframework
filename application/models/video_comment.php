<?php
    ...
    {
        protected $table_name = "videos_comments";
        protected $db_fields  = array("id", "video_id", "user_id", "answer_id",
                                                "comment", "posted_on");
        ...
        public function find_comments_on($video_id      = false,
                                                    $page          = 1,
                                                    $validate_page = true)
        {
            // Making pagination
        ...
        public function count_on($video_id = false)
        {
            $sql = "WHERE video_id = %d";
            $sql = sprintf($sql,
                                $this->database->escape_value($video_id));
        ...
        public function save()
        {
            ...
            $video_stats->increase_comments_count();
            $video_stats->save();
        ...
        public function delete()
        {
            $video_stats_model = new Video_Stats_Model;
            $video_stats       = $video_stats_model->find_stats_on($this->video_id);
            $video_stats->decrease_comments_count();
            $video_stats->save();
            
            return parent::delete();
        }
        
        public function delete_comments_on($video_id)
        {
            $sql = "WHERE video_id = %d";
            $sql = sprintf($sql,
                                $this->database->escape_value($video_id));
        ...
        public function delete_all_by_user($user_id)
        {
            ...
            {
                $video_stats = $video_stats_model->find_stats_on($user_comment->video_id);
                $video_stats->decrease_comments_count();
                $video_stats->save();
            }
